...
├── agents/        # `@g4os/agents/interface` — IAgent + AgentFactory + AgentRegistry (Result) + AgentEvent union + schemas Zod (ADR-0070); `@g4os/agents/claude` — ClaudeAgent modular (mapper + prompt-cache + stream-runner + providers lazy direct/bedrock/compat), ADR-0071; Codex/Pi pendentes (TASK-07-03..04)
