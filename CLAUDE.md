...
├── translate/                # i18n core — TranslationKey type + en-us/pt-br locales (~1170 keys)
...
├── auth/                     # `@g4os/auth/{otp,managed-login,entitlement,refresh,supabase}` — OTP fix V1, ManagedLoginService FSM, EntitlementService, SessionRefresher (ADRs 0091-0094)
├── data/                     # node:sqlite (Node 24) + Drizzle (beta 1.0 pinado, ADR-0042), migrations com backup pré-migration, `events/` (JSONL append-only + replay + checkpoints, ADR-0043), `attachments/` (content-addressed SHA-256 + refcount + GC, ADR-0044), `backup/` (ZIP v1: export/restore + manifest Zod, ADR-0045)
├── observability/            # pino transports (via @g4os/kernel logger), OTel tracer/propagation + lazy SDK, Sentry scrub/init, memory monitor + leak detector, prom-client metrics, debug ZIP export, PostHog opt-in (ADRs 0060–0065)
├── codex-types/              # Types-only do Codex CLI app-server protocol (NDJSON wire format)
├── agents/                   # `@g4os/agents/interface` IAgent + Registry (ADR-0070); `claude` ClaudeAgent (ADR-0071); `codex` CodexAgent via subprocess NDJSON (ADR-0072); `openai` OpenAIAgent (ADR-0074); `google` GoogleAgent native routing (ADR-0075); `shared` broker shared (ADR-0073)
├── sources/                  # `@g4os/sources/{interface,mcp-stdio,mcp-http,managed,oauth,lifecycle,planner,catalog,store,broker}` — ISource + SourceRegistry + MCP + OAuth kit + SourcePlanner + 15 managed seeds (ADRs 0081-0086, 0136-0137, 0143-0144)
├── permissions/              # `@g4os/permissions` — PermissionBroker (Deferred queue) + PermissionStore (SHA-256 args hash, JSON atômico). ADR-0134
├── session-runtime/          # `@g4os/session-runtime` — `runToolLoop` (multi-iteração tool use + permission gate), `runAgentIteration`, `SessionEventBus`, `finalizeAssistantMessage`, event-log wrappers. ADR-0135
├── migration/                # V1→V2 detect/plan/execute (workspaces, sources, sessions, skills, credentials, config). TASK-14-01..04
├── bridge-mcp-server/        # SKELETON — re-expõe session tools como MCP server pra agents externos (CodexAgent). TASK-18-01
├── session-mcp-server/       # SKELETON — re-expõe sessão atual como MCP server (introspection, debug). TASK-18-02
├── usage-reconcile-worker/   # SKELETON — reconcilia tokens locais vs billing backend (gated em multi-user). TASK-18-07
├── release-channels/         # Types + criteria + schedule pra rollout canary→beta→stable + feature flags. TASK-15-04
├── features/                 # scaffolding — Feature-Sliced Design por domínio (TASK-11)
└── ui/                       # React + Radix + Tailwind compartilhado, custom block registry (mermaid, pdf-preview)
...
- **Main process total < 10300 LOC** (gate `check:main-size`), arquivos em `apps/desktop/src/main/` ≤ 300 LOC cada. Histórico completo de bumps + justificativa por bump vive em `scripts/check-main-size.ts` (JSDoc do header — leia antes de bumpar de novo). Resumo: 2000→3000→4500→4800→6200→6500 (Epic 10b/11-workspaces/11-projects/OUTLIER-07-09/FOLLOWUP-04-08) → 6700→6800 (OUTLIER-12 mount + OUTLIER-23 E2E mock) → 6900→7100→7200→7250 (CR-3/5/6/10) → 8800→8900→9000→9300→9400→9600 (CR multi + Epic 18) → 9700→9800 (CR-15/16 fixes) → 10000 (CR-17 — Backup IPC + UI) → 10150 (CR-18 F-DT-I — single-instance lock + setAsDefaultProtocolClient deep-link delivery) → **10300** (code-review-19 retrofit — `services/services-prober.ts` 111 LOC para Settings → Services HTTP probing, introduzido em commit `bd20855` sem bump original; candidato natural pra extração futura em `@g4os/observability/probe`). Próxima elevação exige nova extração estrutural OU ADR justificando.
...
pnpm check:hover-pattern           # gate hover:bg-foreground/N legacy (CLAUDE.md UI patterns)
...
- **FOLLOWUP-CR32-EntitlementWiring** — `EntitlementService` (`packages/auth/src/entitlement/service.ts`) é dead code — ZERO consumidores em `apps/desktop`. API está corretamente exportada e sem bugs internos, mas ADR-0093 declara wiring real (EntitlementClient HTTP + cache TTL + gate no chat send) que nunca foi materializado. V1 tinha `getEntitlements()` real em `user-auth.ts`. Próximo passo: criar `EntitlementClient` real falando com `/api/entitlements`, integrar gate no `TurnDispatcher` (bloquear sem entitlement) ou adotar devBypass explícito com `onBypassUsed` audit. Marcar done somente após wiring + teste de integração.
